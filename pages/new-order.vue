...
<script>
import { createLoad } from "~/api/load";
export default {
  data() {
    return {
      e13: 1,
  ...
  methods: {
    ...
    async updateLoadType() {
      ...
      if (!this.checkStepValidity(this.loadType, this.steps[0].step)) {
        this.$notifier.showMessage({
          content: "Please fill correctly all required fields!",
          color: "error",
        });
        return;
      }
    ...
    updateLoadSpecifications() {
      ...
      if (!this.checkStepValidity(this.specs, this.steps[1].step)) {
        this.$notifier.showMessage({
          content: "Please fill correctly all required fields!",
          color: "error",
        });
    ...
    checkIfLocationsChecked() {
      ...
      if (
        !(
          this.locations[1].lat &&
          this.locations[1].lon &&
          this.locations[1].checked
        )
      ) {
        this.$notifier.showMessage({
    ...
    updateLocations() {
      ...
      if (!this.checkStepValidity(this.locations, this.steps[2].step)) {
        this.$notifier.showMessage({
          content: "Please fill correctly all required fields!",
          color: "error",
        });
    ...
    async updateLoadInfo() {
      ...
      if (!this.checkStepValidity(this.info, this.steps[3].step)) {
        this.$notifier.showMessage({
          content: "Please fill correctly all required fields!",
          color: "error",
        });
      ...
      await this.saveLoad(payload);
      this.$router.push("/");
    },
    async saveLoad(payload) {
      try {
        this.loading = true;
    ...
    checkObjectValidity(obj) {
      ...
      return Object.values(obj).every((element) => element !== null && element);
    },
